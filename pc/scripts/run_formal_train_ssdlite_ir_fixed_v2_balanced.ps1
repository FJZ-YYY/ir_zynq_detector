...
param(
    ...
    [int]$SaveEvery = 5,
    [int]$LogInterval = 50,
    [double]$SamplerEmptyImageWeight = 0.15,
    [double]$SamplerRareClassPower = 1.0,
    [double]$SamplerMaxClassWeight = 3.5,
    [switch]$Amp,
    [switch]$NoResume
)
...
$DefaultOutputDir = Join-Path $RepoRoot "build\train_ssdlite_ir_fixed_v2_balanced"
...
$Args = @(
    ...
    "--sampler", "weighted",
    "--sampler-empty-image-weight", "$SamplerEmptyImageWeight",
    "--sampler-rare-class-power", "$SamplerRareClassPower",
    "--sampler-max-class-weight", "$SamplerMaxClassWeight"
)
...
if ((-not $NoResume.IsPresent) -and (Test-Path $ResumePath)) {
    Write-Host "Resuming balanced fixed-contract training from $ResumePath"
    $Args += @("--resume", $ResumePath)
}

Write-Host "Starting balanced fixed-contract training..."
