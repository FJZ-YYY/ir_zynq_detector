...
if {![file exists $xsa_file]} {
    error "XSA file not found: $xsa_file. Run Vivado build_bitstream.tcl first."
}

if {[file exists $workspace]} {
    puts "INFO: Removing existing generated Vitis raw-sample workspace: $workspace"
...
set debug_dir [file join $workspace $app_name Debug]
set debug_makefile [file join $debug_dir makefile]
set debug_subdir_mk [file join $debug_dir src subdir.mk]

if {![file exists $debug_makefile]} {
    error "Debug makefile not found after app build: $debug_makefile"
}
if {![file exists $debug_subdir_mk]} {
    error "Debug subdir.mk not found after app build: $debug_subdir_mk"
}
...
set fp [open $debug_subdir_mk r]
set subdir_text [read $fp]
close $fp

if {[string first $backend_define $subdir_text] < 0} {
    set needle {-MMD -MP}
    if {[string first $needle $subdir_text] < 0} {
        error "Could not patch backend define into subdir.mk: $debug_subdir_mk"
    }
    set subdir_text [string map [list $needle "$backend_define $needle"] $subdir_text]
    set fp [open $debug_subdir_mk w]
    puts -nonewline $fp $subdir_text
    close $fp
}

set old_pwd [pwd]
cd $debug_dir
if {[catch {exec make clean all 2>@1} build_msg]} {
    cd $old_pwd
...
puts "INFO: Raw-sample backend define: $backend_define"
